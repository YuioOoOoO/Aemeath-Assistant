...
const api = {
  ...
  setForceIgnoreMouse: (forceIgnore: boolean) => {
    ipcRenderer.send('set-force-ignore-mouse', forceIgnore);
  },
  ...
  showContextMenu: (status?: Record<string, boolean>) => {
    console.log('Preload showContextMenu');
    ipcRenderer.send('show-context-menu', status || {});
  },
  ...
  onShowInputSubtitle: (callback: () => void) => {
    const handler = (_event: any) => callback();
    ipcRenderer.on('show-input-subtitle', handler);
    return () => ipcRenderer.removeListener('show-input-subtitle', handler);
  },
  onHideInputSubtitle: (callback: () => void) => {
    const handler = (_event: any) => callback();
    ipcRenderer.on('hide-input-subtitle', handler);
    return () => ipcRenderer.removeListener('hide-input-subtitle', handler);
  },
