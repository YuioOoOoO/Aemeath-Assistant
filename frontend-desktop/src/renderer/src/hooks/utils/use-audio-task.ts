...
interface AudioTaskOptions {
  ...
  motions?: Array<{ group: string; index: number }> | null
  speaker_uid?: string
  forwarded?: boolean
}
...
export const useAudioTask = () => {
  ...
  const { setSubtitleText, setSpeechBubbleText } = useSubtitle();
  ...
  const handleAudioPlayback = (options: AudioTaskOptions): Promise<void> => new Promise((resolve) => {
    ...
    const {
      audioBase64, displayText, expressions, motions, forwarded,
    } = options;
    ...
    if (displayText) {
      ...
      if (audioBase64) {
        ...
        setSpeechBubbleText(displayText.text);
      }
      if (!forwarded) {
        sendMessage({
    ...
    try {
      ...
      if (audioBase64) {
        ...
        let voiceVisualsStarted = false;

        const cleanup = () => {
          // ended/error can both fire for the same element. A late duplicate
          // from an older segment must never unlock the newer segment's mouth.
          if (isFinished) return;
          isFinished = true;
          const wasCurrentAudio = audioManager.clearCurrentAudio(audio);
          if (wasCurrentAudio) {
            // Keep ownership until the smoothed mouth reaches its neutral
            // closed value; update() releases it based on state, not a timer.
            model._voiceLipSyncReleasing = true;
          }
          resolve();
          window.setTimeout(() => {
            if (!audioTaskQueue.hasTask() && !audioManager.hasCurrentAudio()) {
              setSpeechBubbleText('');
            }
          }, 3000);
        };
        ...
        audio.addEventListener('canplaythrough', () => {
          ...
          if (voiceVisualsStarted) return;
          voiceVisualsStarted = true;

          // Voice takes ownership only when playback is actually ready to
          // begin. Until this point mouse interactions remain authoritative.
          model.beginVoiceInteraction?.();
          model._voiceLipSyncActive = true;
          model._voiceLipSyncReleasing = false;
          ...
          if (voiceMotion) {
            const finishVoicePerformance = audioManager.beginVoicePerformance();
            model.startMotion(
              voiceMotion.group,
              voiceMotion.index,
              LAppDefine.PriorityForce,
              finishVoicePerformance,
              false,
              true,
            );
          } else {
            audioManager.completeVoicePerformance();
