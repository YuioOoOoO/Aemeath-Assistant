...
export const useLive2DResize = ({
  ...
  const scaleStorageKey = `live2dScale:${mode}:${modelId}`;
  const loadStoredScale = useCallback(() => {
    try {
      const savedScale = Number(window.localStorage.getItem(scaleStorageKey));
      if (Number.isFinite(savedScale) && savedScale >= MIN_SCALE && savedScale <= MAX_SCALE) {
        return savedScale;
      }
    } catch (error) {
      console.warn('[Live2D] Failed to restore model scale:', error);
    }
    return undefined;
  }, [scaleStorageKey]);

  // Initialize scale references
  const initialScale = loadStoredScale() ?? modelInfo?.kScale ?? DEFAULT_SCALE;
   ...
   */
  useEffect(() => {
    const newInitialScale = loadStoredScale() ?? modelInfo?.kScale ?? DEFAULT_SCALE;
    ...
    const scaleRestoreTimers = [250, 800, 1500].map((delay) => setTimeout(() => {
      applyScale(newInitialScale);
    }, delay));

    return () => {
      cancelAnimationFrame(resizeHandle);
      scaleRestoreTimers.forEach(clearTimeout);
    };
  }, [mode, modelInfo?.url, modelInfo?.kScale, loadStoredScale]);
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    try {
      window.localStorage.setItem(scaleStorageKey, String(newTargetScale));
    } catch (error) {
      console.warn('[Live2D] Failed to save model scale:', error);
    }
  ...
  }, [modelInfo?.scrollToResize, animateEase, scaleStorageKey]);
