...
export const useLive2DModel = ({
  ...
  const windowModePositionRef = useRef<Position | null>(null);
  const previousModeRef = useRef(mode);
  const prevModelUrlRef = useRef<string | null>(null);
  const isHoveringModelRef = useRef(false);
  const electronApi = (window as any).electron;

  const getPositionStorageKey = useCallback((targetMode: 'window' | 'pet') => {
    const modelId = modelInfo?.name || modelInfo?.url || 'default';
    return `live2dPosition:${targetMode}:${modelId}`;
  }, [modelInfo?.name, modelInfo?.url]);

  const loadStoredPosition = useCallback((targetMode: 'window' | 'pet'): Position | null => {
    try {
      const saved = JSON.parse(window.localStorage.getItem(getPositionStorageKey(targetMode)) || 'null');
      if (Number.isFinite(saved?.x) && Number.isFinite(saved?.y)) return saved;
    } catch (error) {
      console.warn('[Live2D] Failed to restore model position:', error);
    }
    return null;
  }, [getPositionStorageKey]);

  const saveStoredPosition = useCallback((targetMode: 'window' | 'pet', nextPosition: Position) => {
    if (!Number.isFinite(nextPosition.x) || !Number.isFinite(nextPosition.y)) return;
    try {
      window.localStorage.setItem(
        getPositionStorageKey(targetMode),
        JSON.stringify(nextPosition),
      );
    } catch (error) {
      console.warn('[Live2D] Failed to save model position:', error);
    }
  }, [getPositionStorageKey]);
  ...
  useEffect(() => {
    const restorePosition = () => {
      const savedPosition = loadStoredPosition(mode);
      const currentPosition = getModelPosition();
      const nextPosition = savedPosition || currentPosition;
      if (savedPosition) setModelPosition(savedPosition.x, savedPosition.y);
      modelPositionRef.current = nextPosition;
      setPosition(nextPosition);
      if (mode === 'window') windowModePositionRef.current = nextPosition;
    };

    // Model asset loading is asynchronous. Restore more than once so the
    // saved transform wins even when the SDK finishes loading a little later.
    const timers = [500, 1000, 1600].map((delay) => setTimeout(restorePosition, delay));
    return () => timers.forEach(clearTimeout);
  }, [modelInfo?.url, mode, getModelPosition, loadStoredPosition, setModelPosition]);

  // Pet mode uses the whole virtual desktop as its canvas and lets the model
  // be dragged within that coordinate space. Reusing that translation in the
  // much smaller window canvas can leave the model completely off-screen.
  // Preserve the window-space position before entering pet mode and restore it
  // only after the window canvas has been resized back to its normal bounds.
  useEffect(() => {
    ...
    const positionBeforeModeChange = getModelPosition();
    saveStoredPosition(previousMode, positionBeforeModeChange);

    if (previousMode === 'window' && mode === 'pet') {
      windowModePositionRef.current = positionBeforeModeChange;
    }

    const restoreForCurrentMode = () => {
      const restoredPosition = loadStoredPosition(mode)
        || (mode === 'window' ? windowModePositionRef.current : null);
      if (!restoredPosition) return;
      setModelPosition(restoredPosition.x, restoredPosition.y);
      modelPositionRef.current = restoredPosition;
      setPosition(restoredPosition);
    };

    let secondFrame = 0;
    const firstFrame = requestAnimationFrame(() => {
      secondFrame = requestAnimationFrame(restoreForCurrentMode);
    });
    const retryTimers = [500, 1100].map((delay) => setTimeout(restoreForCurrentMode, delay));
  ...
  }, [mode, getModelPosition, loadStoredPosition, saveStoredPosition, setModelPosition]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const hitAreaName = model.anyhitTest(
      modelX,
      modelY,
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isPet && !isDragging && !isPotentialTapRef.current && electronApi && adapter && view && model && canvasRef.current) {
      ...
      const currentHitState =
        model.anyhitTest(modelX, modelY, modelInfo?.tapMotions) !== null ||
    ...
    if (isDragging) {
      ...
      if (adapter) {
        ...
        if (currentModel && currentModel._modelMatrix) {
          ...
          saveStoredPosition(mode, finalPos);
        }
      }
  ...
  }, [isDragging, canvasRef, modelInfo, mode, saveStoredPosition]);
