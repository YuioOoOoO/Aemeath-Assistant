    ...
    >
      ...
      >
        <SelectTrigger {...settingStyles.general.select.trigger}>
          <SelectValueText placeholder={placeholder} color="#FFF7FA" />
        </SelectTrigger>
        <SelectContent
          bg="linear-gradient(155deg, rgba(75, 49, 68, 0.99), rgba(48, 34, 51, 0.99))"
          color="#FFF7FA"
          border="1px solid rgba(249, 205, 222, 0.34)"
          borderRadius="14px"
          boxShadow="0 18px 48px rgba(10, 5, 14, 0.52), inset 0 1px 0 rgba(255,255,255,0.07)"
          p="6px"
          css={{
            scrollbarWidth: 'thin',
        ...
        >
          {collection.items.map((item) => (
            <SelectItem
              ...
              borderRadius="10px"
              px="12px"
              _hover={{ bg: 'rgba(236, 169, 199, 0.18)' }}
              _highlighted={{ bg: 'rgba(236, 169, 199, 0.22)' }}
              _checked={{ bg: 'rgba(211, 132, 169, 0.24)', color: '#FFF9FC' }}
            >
              {item.label}
            </SelectItem>
          ))}
