...
interface Live2DUserPreferences {
  pointerInteractive?: boolean;
  scrollToResize?: boolean;
}
...
export function Live2DConfigProvider({ children }: { children: React.ReactNode }) {
  ...
  // Keep user-editable switches separate from server model metadata. The
  // server re-sends modelInfo after every reconnect and would otherwise
  // overwrite the values restored from local storage.
  const [userPreferences, setUserPreferences] = useLocalStorage<Live2DUserPreferences>(
    'live2dUserPreferences',
    {},
  );
  ...
  const setModelInfo = (info: ModelInfo | undefined) => {
    ...
    // Server configuration scale needs the desktop multiplier once. Updates
    // to switches/preferences for the currently loaded model must retain the
    // already-normalized value instead of doubling it on every save.
    const isCurrentModel = Boolean(modelInfo?.url && modelInfo.url === info.url);
    const finalScale = isCurrentModel
      ? Number(info.kScale || modelInfo?.kScale || 1)
      : Number(info.kScale || 0.5) * 2;
    console.log("Setting model info with default scale:", finalScale);

    const nextPreferences = isCurrentModel
      ? {
        pointerInteractive: info.pointerInteractive ?? modelInfo?.pointerInteractive ?? true,
        scrollToResize: info.scrollToResize ?? modelInfo?.scrollToResize ?? true,
      }
      : userPreferences;
    ...
    setModelInfoState({
      ...info,
      kScale: finalScale,
      pointerInteractive: nextPreferences.pointerInteractive
        ?? info.pointerInteractive
        ?? true,
      scrollToResize: nextPreferences.scrollToResize
        ?? info.scrollToResize
        ?? true,
    });
  };
