...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    if (this._lipsync) {
      ...
      // Slightly enlarge the speaking mouth while keeping the model's valid
      // parameter range and the existing time-based smoothing unchanged.
      value = Math.min(1.0, value * 2.6);

      if (this._voiceLipSyncActive) {
        if (this._voiceLipSyncReleasing) {
          value = 0.0;
        }
        // Smooth both opening and closing using time-based interpolation. The
        // slightly slower release prevents rapid syllable-level snapping while
        // still allowing visible pauses between phrases.
        const smoothingSeconds = value > this._voiceLipSyncValue ? 0.10 : 0.18;
        const smoothingFactor = 1.0 - Math.exp(
          -deltaTimeSeconds / smoothingSeconds
        );
        this._voiceLipSyncValue += (
          value - this._voiceLipSyncValue
        ) * smoothingFactor;

        // Voice lip sync is the final authority over mouth parameters while
        // audio is playing. Native emotion/body motions still animate every
        // other parameter, but their authored mouth pose cannot cover speech.
        const mouthFormId = CubismFramework.getIdManager().getId(
          CubismDefaultParameterId.ParamMouthForm
        );
        if (this._model.getParameterIndex(mouthFormId) !== -1) {
          this._model.setParameterValueById(mouthFormId, 0.0);
        }
        ...
        // Hand mouth parameters back only after the speaking mouth has
        // naturally reached its closed neutral value. No fixed timeout is used.
        if (
          this._voiceLipSyncReleasing &&
          this._voiceLipSyncValue <= 0.01
        ) {
          this._voiceLipSyncValue = 0.0;
          this._voiceLipSyncReleasing = false;
          this._voiceLipSyncActive = false;
        }
      } else {
        for (let i = 0; i < this._lipSyncIds.getSize(); ++i) {
          this._model.addParameterValueById(this._lipSyncIds.at(i), value, 4.0);
        }
      }
    }
    ...
    if (motion == null) {
      ...
      fetch(`${this._modelHomeDir}${motionFileName}`)
        ...
        .then((arrayBuffer) => {
          ...
          // Mouse interactions must play the model-authored curves verbatim.
          // CubismMotion defaults missing fade metadata to one second; because
          // the model parameters are saved every frame, that implicit fade can
          // leave the last pre-neutral eye value behind when the motion ends.
          if (!useMotionFade) {
            motion.setFadeInTime(0.0);
            motion.setFadeOutTime(0.0);
          }
          // Voice performances must finish after one model-authored cycle so
          // conversation cleanup can run. Mouse/idle motions retain the Loop
          // value authored in the motion3 file.
          motion.setIsLoop(playOnce ? false : motion._motionData.loop);
  ...
  /** Stop the active expression and return to the model's authored base face. */
  public resetExpression(): void {
    this._expressionManager?.stopAllMotions();
  }

  /**
   * Hit-test a drawable with proportional padding around its authored bounds.
   * Used sparingly for very small controls such as the cat-ear toggle ornament.
   */
  private isHitWithPadding(
    ...
    const paddingX = (right - left) * paddingRatio;
    const paddingY = (bottom - top) * paddingRatio;
    const modelX = this._modelMatrix.invertTransformX(pointX);
    const modelY = this._modelMatrix.invertTransformY(pointY);

    return (
      modelX >= left - paddingX &&
      modelX <= right + paddingX &&
      modelY >= top - paddingY &&
      modelY <= bottom + paddingY
    );
  }
  ...
  public anyhitTest(
    x: number,
    y: number,
    configuredHitAreas?: Record<string, unknown>
  ): string | null {
    ...
    const count: number = this._modelSetting.getHitAreasCount();
    const renderOrders = this._model.getDrawableRenderOrders();
    let topmostHitArea: string | null = null;
    let topmostRenderOrder = Number.NEGATIVE_INFINITY;

    for (let i = 0; i < count; i++) {
      const hitAreaName = this._modelSetting.getHitAreaName(i);

      // Pointer actions should only compete with other configured pointer
      // actions. Parameter-only areas such as 刘海R (head stroking) may overlap
      // a button but must not swallow that button's click.
      if (
        configuredHitAreas &&
        !Object.prototype.hasOwnProperty.call(configuredHitAreas, hitAreaName)
      ) {
        continue;
      }
      ...
      // Toggle/cancel controls in the spreadsheet are attached to parts that
      // only exist in particular model states (cat, sword, controller, wings).
      // Invisible drawables must not intercept clicks before they are shown.
      if (
        drawIndex < 0 ||
        !this._model.getDrawableDynamicFlagIsVisible(drawIndex) ||
        this._model.getDrawableOpacity(drawIndex) <= 0.01
      ) {
        continue;
      }

      const isCatEarToggle = hitAreaName.trim() === '头饰金属L';
      const hit = isCatEarToggle
        ? this.isHitWithPadding(drawId, x, y, 0.3)
        : this.isHit(drawId, x, y);

      // The enlarged cat-ear toggle overlaps ArtMesh74 (左侧发). Within this
      // deliberately expanded control, the toggle must win; otherwise the
      // higher-rendered hair mesh starts 食指/戳头/摇头 instead.
      if (isCatEarToggle && hit) {
        if (LAppDefine.DebugLogEnable) {
          console.log('[APP] anyhitTest: Cat-ear toggle priority hit.');
        }
        return hitAreaName;
      }

      if (hit) {
        const renderOrder = renderOrders[drawIndex];

        if (renderOrder >= topmostRenderOrder) {
          topmostRenderOrder = renderOrder;
          topmostHitArea = hitAreaName;
        }
      }
    }

    if (topmostHitArea && LAppDefine.DebugLogEnable) {
      console.log(`[APP] anyhitTest: Topmost visible hit area: ${topmostHitArea}`);
    }

    return topmostHitArea;
  }
  ...
  public startTapMotion(hitAreaName: string | null, tapMotionsConfig: any): void {
    ...
    // 2. Only spreadsheet-configured hit areas may trigger interactions.
    // Clicking transparent/unmapped model parts must not choose a random action
    // from an unrelated body area.
    if (!areaSpecificHit) {
      if (LAppDefine.DebugLogEnable) {
        console.log('[APP] startTapMotion: Hit area is not configured; ignoring tap.');
      }
      return;
    }

    // Voice owns the model only while speech is actually playing and until
    // its final performance has returned to Idle. All other time belongs to
    // pointer interaction.
    if (this._voiceInteractionActive) {
      if (LAppDefine.DebugLogEnable) {
        console.log('[APP] startTapMotion: Voice interaction is active; ignoring tap.');
      }
      return;
    }
    ...
    // 5. Play the selected native motion group. PriorityNormal can interrupt
    // Idle, while Cubism rejects it if another non-idle interaction is active.
  ...
  _voiceLipSyncActive = false;
  _voiceLipSyncReleasing = false;
  _voiceLipSyncValue = 0.0;
  _voiceInteractionActive = false;

  public beginVoiceInteraction(): void {
    this._voiceInteractionActive = true;
  }

  public endVoiceInteraction(): void {
    if (!this._voiceInteractionActive) return;
    this._motionManager.stopAllMotions();
    this.startRandomMotion(
      LAppDefine.MotionGroupIdle,
      LAppDefine.PriorityIdle
    );
    this._voiceInteractionActive = false;
  }
