...
class Live2dModel:
    ...
    def set_model(self, model_name: str) -> None:
        ...
        self.voice_emotion_map: dict = self.model_info.get("voiceEmotionMap", {})
    ...
    def split_emotion_segments(self, text: str) -> list[tuple[str, str | None, int]]:
        """Split text at [emotion] or [emotion:1-5] markers.

        Text before the first marker remains neutral. Markers are removed so
        they never reach subtitles or TTS.
        """
        keys = "|".join(re.escape(key) for key in self.emo_map)
        if not keys:
            return [(text, None, 3)]
        marker = re.compile(rf"\[({keys})(?::([1-5]))?\]", re.IGNORECASE)
        ...
        if matches[0].start() > 0 and text[: matches[0].start()].strip():
            segments.append((text[: matches[0].start()].strip(), None, 3))
        for index, match in enumerate(matches):
            end = matches[index + 1].start() if index + 1 < len(matches) else len(text)
            segment_text = text[match.end() : end].strip()
            if segment_text:
                segments.append(
                    (
                        segment_text,
                        match.group(1).lower(),
                        int(match.group(2) or 3),
                    )
                )
        return segments

    def select_voice_performance(
        self, emotion: str | None, intensity: int
    ) -> tuple[list, list[dict]]:
        """Select one calibrated expression and optional native motion."""
        if not emotion:
            return [], []
        profile = self.voice_emotion_map.get(emotion, {})
        tier_name = "weak" if intensity <= 2 else "medium" if intensity == 3 else "strong"
        tier = profile.get(tier_name, profile.get("medium", {}))
        expressions = tier.get("expressions", [])
        motions = tier.get("motions", [])
        selected_expression = [random.choice(expressions)] if expressions else []
        selected_motion = [random.choice(motions)] if motions else []
        return selected_expression, selected_motion
