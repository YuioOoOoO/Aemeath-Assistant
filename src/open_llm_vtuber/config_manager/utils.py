...
def scan_config_alts_directory(config_alts_dir: str) -> list[dict]:
    ...
    config_files.append({"filename": "conf.yaml", "name": default_name})
    seen_names.add(default_name.casefold())
        ...
        for file in files:
            if file.endswith(".yaml"):
                ...
                if normalized_name in seen_names:
                    logger.debug(
                        f"Skipping duplicate character preset '{display_name}' from {file}"
                    )
                    continue
                seen_names.add(normalized_name)
                config_files.append({"filename": file, "name": display_name})
