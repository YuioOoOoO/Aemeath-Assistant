...
class VolcengineTTSConfig(I18nMixin):
    ...
    sample_rate: Literal[8000, 16000, 22050, 24000, 32000, 44100, 48000] = Field(
        24000, alias="sample_rate"
    )
    speech_rate: int = Field(0, ge=-50, le=100, alias="speech_rate")
    loudness_rate: int = Field(0, ge=-50, le=100, alias="loudness_rate")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_key": Description(en="Volcengine API key", zh="火山引擎 API Key"),
        "resource_id": Description(
            en="Volcengine Seed-TTS resource ID", zh="火山引擎 Seed-TTS 资源 ID"
        ),
        "speaker": Description(
            en="Exact Volcengine speaker ID", zh="火山引擎准确的 Speaker ID"
        ),
        "endpoint": Description(
            en="Volcengine V3 WebSocket endpoint", zh="火山引擎 V3 WebSocket 地址"
        ),
        "sample_rate": Description(en="Audio sample rate", zh="音频采样率"),
        "speech_rate": Description(
            en="Speech rate adjustment (-50 to 100)", zh="语速调整（-50 到 100）"
        ),
        "loudness_rate": Description(
            en="Loudness adjustment (-50 to 100)", zh="音量调整（-50 到 100）"
        ),
        "emotion_enabled": Description(
            en="Enable emotion control from Live2D expression tags",
            zh="根据 Live2D 表情标签启用情绪控制",
        ),
        "emotion_scale": Description(
            en="Emotion intensity (1 to 5)", zh="情绪强度（1 到 5）"
        ),
        "emotion_map": Description(
            en="Mapping from Live2D emotion keys to Volcengine emotions",
            zh="Live2D 情绪名称到火山引擎情绪名称的映射",
        ),
        "timeout_seconds": Description(
            en="Request timeout in seconds", zh="请求超时秒数"
        ),
    }
...
class TTSConfig(I18nMixin):
    ...
    volcengine_tts: Optional[VolcengineTTSConfig] = Field(None, alias="volcengine_tts")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "volcengine_tts": Description(
            en="Configuration for Volcengine Seed-TTS", zh="火山引擎 Seed-TTS 配置"
        ),
