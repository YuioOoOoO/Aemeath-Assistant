            ...
            async for item in stream:
                if isinstance(item, SentenceWithTags):
                    ...
                    if not any(
                        ...
                        for segment_text, emotion, intensity in live2d_model.split_emotion_segments(
                            sentence.text
                        ):
                            actions = Actions()
                            if emotion:
                                expressions, motions = live2d_model.select_voice_performance(
                                    emotion, intensity
                                )
                                actions.expressions = expressions or None
                                actions.motions = motions or None
                                actions.emotion = emotion
                                actions.emotion_intensity = intensity
                            yield SentenceWithTags(segment_text, sentence.tags), actions
                    else:
