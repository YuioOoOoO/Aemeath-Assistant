...
async def finalize_conversation_turn(
    ...
    if tts_manager.task_list:
        # TTS generation runs in parallel, while its payload sender is a
        # separate task. Wait for both generation and ordered delivery before
        # sending completion/control messages on the same WebSocket.
        await tts_manager.wait_until_delivered()
        ...
        if not response:
            ...
            # Do not leave the frontend in thinking-speaking forever. The end
            # signal is still required even when the playback acknowledgement
            # is lost or times out.
