...
class TTSTaskManager:
    ...
    async def wait_until_delivered(self) -> None:
        """Wait until generation and ordered WebSocket delivery both finish."""
        if self.task_list:
    ...
    async def _process_tts(
        ...
        try:
            emotion_key = self._resolve_emotion(actions, live2d_model)
            emotion_intensity = actions.emotion_intensity if actions else None
            audio_file_path = await self._generate_audio(
                tts_engine, tts_text, emotion_key, emotion_intensity
            )
    ...
    def _resolve_emotion(
        actions: Optional[Actions], live2d_model: Live2dModel
    ) -> Optional[str]:
        """Resolve the selected Live2D expression to its emotion key."""
        if not actions or not actions.expressions:
            return actions.emotion if actions else None
        if actions.emotion:
            return actions.emotion
        expression = actions.expressions[-1]
        if isinstance(expression, str) and expression in live2d_model.emo_map:
            return expression
        for emotion_key, expression_value in live2d_model.emo_map.items():
            if expression_value == expression:
                return emotion_key
        return None
    ...
    async def _generate_audio(
        ...
        emotional_generator = getattr(
            tts_engine, "async_generate_audio_with_emotion", None
        )
        if emotional_generator is not None:
            return await emotional_generator(
                text=text,
