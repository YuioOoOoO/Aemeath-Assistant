...
class WebSocketHandler:
    ...
    def _init_message_handlers(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            "fetch-model-config": self._handle_fetch_model_config,
            "update-model-config": self._handle_update_model_config,
    ...
    _TTS_PROVIDERS = {
        "volcengine_tts", "openai_tts", "siliconflow_tts", "fish_api_tts",
    }

    @staticmethod
    def _masked_secret(value: Optional[str]) -> dict:
        return {"configured": bool(value and not value.lower().startswith(("your ", "not-needed")))}

    @classmethod
    def _public_model_config(cls, context: ServiceContext) -> dict:
        basic = context.character_config.agent_config.agent_settings.basic_memory_agent
        llm_provider = basic.llm_provider
        llm = getattr(context.character_config.agent_config.llm_configs, llm_provider)
        tts_config = context.character_config.tts_config
        tts_provider = tts_config.tts_model
        tts = getattr(tts_config, tts_provider)
        llm_data = llm.model_dump() if llm else {}
        tts_data = tts.model_dump() if tts else {}
        for data in (llm_data, tts_data):
            for key in list(data):
                if "key" in key.lower() or "secret" in key.lower():
                    data[key] = cls._masked_secret(data[key])
        def public_pool(owner, providers):
            pool = {}
            for provider in providers:
                item = getattr(owner, provider, None)
                values = item.model_dump() if item else {}
                for key in list(values):
                    if "key" in key.lower() or "secret" in key.lower():
                        values[key] = cls._masked_secret(values[key])
                pool[provider] = values
            return pool
        ...
        return {
            ...
            "tts": tts_data,
            "llm_providers": sorted(cls._LLM_PROVIDERS),
            "tts_providers": sorted(cls._TTS_PROVIDERS),
            "llm_configs": public_pool(context.character_config.agent_config.llm_configs, cls._LLM_PROVIDERS),
            "tts_configs": public_pool(tts_config, cls._TTS_PROVIDERS),
        }

    async def _handle_fetch_model_config(self, websocket, client_uid, data) -> None:
        await websocket.send_text(json.dumps({
            "type": "model-config",
            "config": self._public_model_config(self.client_contexts[client_uid]),
        }))

    async def _handle_update_model_config(self, websocket, client_uid, data) -> None:
        """Apply model credentials without ever sending stored secrets to the UI."""
        context = self.client_contexts[client_uid]
        requested = data.get("config") or {}
        llm_provider = requested.get("llm_provider")
        tts_provider = requested.get("tts_provider")
        if llm_provider not in self._LLM_PROVIDERS or tts_provider not in self._TTS_PROVIDERS:
            raise ValueError("不支持的模型运营商")

        config_data = read_yaml("conf.yaml")
        character = config_data["character_config"]
        basic_yaml = character["agent_config"]["agent_settings"]["basic_memory_agent"]
        llm_yaml = character["agent_config"]["llm_configs"][llm_provider]
        tts_yaml = character["tts_config"][tts_provider]

        def merge_editable(target: dict, updates: dict, allowed: set[str]) -> None:
            for key, value in updates.items():
                if key not in allowed or isinstance(value, dict):
                    continue
                if ("key" in key.lower() or "secret" in key.lower()) and not value:
                    continue
                target[key] = value

        merge_editable(llm_yaml, requested.get("llm") or {}, {
            "base_url", "llm_api_key", "model", "temperature",
        })
        tts_allowed = {
            "api_key", "base_url", "api_url", "model", "default_model", "voice",
            "default_voice", "speaker", "resource_id", "endpoint", "reference_id",
            "latency", "sample_rate", "speech_rate", "loudness_rate",
            "emotion_enabled", "emotion_scale", "timeout_seconds", "file_extension",
            "speed", "gain", "response_format", "stream",
        }
        merge_editable(tts_yaml, requested.get("tts") or {}, tts_allowed)
        basic_yaml["llm_provider"] = llm_provider
        character["tts_config"]["tts_model"] = tts_provider

        # Validate and initialize before persisting, so a bad entry cannot break startup.
        from .config_manager import Config
        new_config = Config.model_validate(config_data)
        new_agent = new_config.character_config.agent_config
        new_tts = new_config.character_config.tts_config
        context.init_tts(new_tts)
        await context.init_agent(new_agent, context.character_config.persona_prompt)
        context.config.character_config.agent_config = new_agent
        context.config.character_config.tts_config = new_tts
        self.default_context_cache.init_tts(new_tts.model_copy(deep=True))
        await self.default_context_cache.init_agent(
            new_agent.model_copy(deep=True),
            self.default_context_cache.character_config.persona_prompt,
        )
        self.default_context_cache.config.character_config.agent_config = new_agent.model_copy(deep=True)
        self.default_context_cache.config.character_config.tts_config = new_tts.model_copy(deep=True)

        with open("conf.yaml", "w", encoding="utf-8") as config_file:
            yaml.safe_dump(config_data, config_file, allow_unicode=True, sort_keys=False)
        await websocket.send_text(json.dumps({
            "type": "model-config-updated", "success": True,
            "config": self._public_model_config(context),
        }))

    @staticmethod
    def _public_tts_config(context: ServiceContext) -> dict:
        """Return editable TTS settings without exposing service credentials."""
        tts_config = context.character_config.tts_config
        result = {"tts_model": tts_config.tts_model}
        if tts_config.tts_model == "volcengine_tts":
            volc = tts_config.volcengine_tts
            result["volcengine_tts"] = {
                "speaker": volc.speaker,
                "sample_rate": volc.sample_rate,
                "speech_rate": volc.speech_rate,
                "loudness_rate": volc.loudness_rate,
                "emotion_enabled": volc.emotion_enabled,
                "emotion_scale": volc.emotion_scale,
                "timeout_seconds": volc.timeout_seconds,
            }
        return result
    ...
    async def _handle_fetch_tts_config(
        ...
        await websocket.send_text(
            json.dumps(
                {"type": "tts-config", "config": self._public_tts_config(context)}
            )
        )
    ...
    async def _handle_update_tts_config(
        ...
        """Validate, apply and persist safe, user-editable TTS settings."""
        context = self.client_contexts[client_uid]
        requested = data.get("config") or {}
        if context.character_config.tts_config.tts_model != "volcengine_tts":
            raise ValueError("当前仅支持在界面中调整火山引擎语音合成")
        ...
        current_tts_data = context.character_config.tts_config.model_dump()
        current_tts_data["volcengine_tts"].update(updates)
        new_tts_config = TTSConfig.model_validate(current_tts_data)

        # Initialize first so invalid or unusable settings never get persisted.
        context.init_tts(new_tts_config)
        context.config.character_config.tts_config = new_tts_config

        # Keep new sessions consistent with this session.
        self.default_context_cache.init_tts(new_tts_config.model_copy(deep=True))
        self.default_context_cache.config.character_config.tts_config = (
            new_tts_config.model_copy(deep=True)
        )

        config_data = read_yaml("conf.yaml")
        volc_yaml = config_data["character_config"]["tts_config"]["volcengine_tts"]
        volc_yaml.update(updates)
        with open("conf.yaml", "w", encoding="utf-8") as config_file:
            yaml.safe_dump(
                config_data,
                config_file,
        ...
        await websocket.send_text(
            json.dumps(
                {
                    "type": "tts-config-updated",
                    "success": True,
                    "config": self._public_tts_config(context),
                }
            )
        )
